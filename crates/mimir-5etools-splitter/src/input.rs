...
impl InputSource {
    ...
    pub fn parse(input: &str) -> Result<Self> {
        ...
        if input.starts_with("https://github.com/") || input.starts_with("git@github.com:") {
            ...
            Err(anyhow!(
                "Only GitHub URLs are supported. Got: {}",
                input
            ))
        } else {
            // Treat as local path
    ...
    pub fn is_2014_ruleset(&self) -> bool {
        match self {
            InputSource::LocalPath(path) => {
                path.to_string_lossy().contains("2014")
            }
            InputSource::GitHub { url, .. } => {
                url.contains("2014")
            }
        }
    }
...
mod tests {
    ...
    fn test_detect_2014_ruleset() {
        let source = InputSource::parse("https://github.com/5etools-mirror-3/5etools-2014-src.git").unwrap();
        assert!(source.is_2014_ruleset());
        
        let source = InputSource::parse("https://github.com/5etools-mirror-3/5etools-src.git").unwrap();
        assert!(!source.is_2014_ruleset());
    }
    ...
    fn test_get_image_repo_url() {
        ...
        let source = InputSource::parse("https://github.com/5etools-mirror-3/5etools-src.git@v1.210.46").unwrap();
        assert_eq!(
            source.get_image_repo_url(),
