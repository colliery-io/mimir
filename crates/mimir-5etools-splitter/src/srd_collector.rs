    ...
    pub fn new() -> Self {
        Self {
            ...
            metadata: SrdMetadata {
                ...
                description: "Open Game Content from the D&D 5th Edition System Reference Document".to_string(),
...
"#;
        ...
        self.files.insert("LICENSE.txt".to_string(), legal_notice.as_bytes().to_vec());
...
pub fn collect_srd_content(repo_path: &Path) -> Result<SrdContent> {
    ...
    for (content_type, items) in all_srd_content {
        if !items.is_empty() {
            ...
            // Update metadata
            content.metadata.content_summary.insert(content_type, items.len());
