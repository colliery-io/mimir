...
pub fn load_all_books(repo_path: &Path) -> Result<Vec<Book>> {
    ...
    let books_json: BooksJson = serde_json::from_str(&content)
...
pub fn load_adventures(repo_path: &Path) -> Result<HashMap<String, serde_json::Value>> {
    ...
    let content = fs::read_to_string(&adventures_file)
        .context("Failed to read adventures.json")?;
    
    let data: serde_json::Value = serde_json::from_str(&content)
        .context("Failed to parse adventures.json")?;
...
pub fn filter_by_source(data: &serde_json::Value, source: &str, key: &str) -> Vec<serde_json::Value> {
...
pub fn get_matching_files(repo_path: &Path, dir: &str, prefix: &str, source: &str) -> Vec<std::path::PathBuf> {
