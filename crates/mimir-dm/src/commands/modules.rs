...
pub async fn create_module(
    conn: State<'_, Mutex<DbConnection>>,
    request: CreateModuleRequest,
) -> Result<ApiResponse<Module>, String> {
    info!("Creating module: {} for campaign {}", request.name, request.campaign_id);
    
    let mut conn = conn.lock().map_err(|e| e.to_string())?;
    let mut service = ModuleService::new(&mut *conn);
    
    match service.create_module(
        request.campaign_id,
        request.name,
...
pub async fn list_campaign_modules(
    ...
    info!("Listing modules for campaign: {}", campaign_id);
    
    let mut conn = conn.lock().map_err(|e| e.to_string())?;
    let mut service = ModuleService::new(&mut *conn);
    
    match service.list_campaign_modules(campaign_id) {
        Ok(modules) => {
...
pub async fn get_module_documents(
    ...
    info!("Getting documents for module: {}", module_id);
    
    let mut conn = conn.lock().map_err(|e| e.to_string())?;
    let mut service = ModuleService::new(&mut *conn);
    
    match service.get_module_documents(module_id) {
        Ok(documents) => {
