...
import { formatEntries } from '../textFormatting'
import { invoke } from '@tauri-apps/api/core'

interface Monster extends MonsterSummary {
  // Ability scores
  str?: number
  dex?: number
  con?: number
  ...
  // Raw data fields (before formatting)
  type?: any  // Raw creature type (can be object)
...
export async function formatMonsterDetails(monster: Monster | MonsterSummary): Promise<string> {
  ...
  html += `<div class="monster-type" style="font-size: 1rem; font-style: italic;">${formatSize(fullMonster.size)} ${creatureType}, ${formatAlignment(fullMonster.alignment)}</div>`
  ...
  if (fullMonster.senses && fullMonster.senses.length > 0) {
    details.push(`<strong>Senses:</strong> ${fullMonster.senses.join(', ')}`)
  }
  if (fullMonster.languages && fullMonster.languages.length > 0) {
    details.push(`<strong>Languages:</strong> ${fullMonster.languages.join(', ')}`)
  }
