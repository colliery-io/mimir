...
import { processFormatting } from '../formatters/textFormatter'
 ...
 */
export function renderEntry(entry: BookEntry, index: number = 0, depth: number = 0): string {
  ...
  if (complexEntry.type === 'section') {
    ...
    return `
      <div class="section" id="${currentId}">
        <${headerTag}>${processFormatting(complexEntry.name || '')}</${headerTag}>
  ...
  if (complexEntry.type === 'entries') {
    ...
    if (complexEntry.name) {
      ...
      headerTag = `<${headerLevel}>${processFormatting(complexEntry.name)}</${headerLevel}>`
    }
    return `
  ...
  if (complexEntry.type === 'inset') {
    return `
      <div class="inset">
        ${complexEntry.name ? `<h4>${processFormatting(complexEntry.name)}</h4>` : ''}
        ${complexEntry.entries ? complexEntry.entries.map((e, i) => renderEntry(e, i, depth + 1)).join('') : ''}
      </div>
    `
  }
    ...
    return `
      <ul class="content-list">
        ${complexEntry.items ? complexEntry.items.map((item, i) => 
          `<li>${typeof item === 'string' ? processFormatting(item) : renderEntry(item, i, depth + 1)}</li>`
        ).join('') : ''}
      </ul>
    `
  }
...
function renderTable(table: ComplexEntry): string {
  ...
  // Caption
  if (table.caption) {
    html += `<caption>${processFormatting(table.caption)}</caption>`
  }
  ...
  if (table.colLabels && table.colLabels.length > 0) {
    ...
    for (const label of table.colLabels) {
      html += `<th>${processFormatting(label)}</th>`
    }
    html += '</tr></thead>'
  }
...
function renderTableCell(cell: any): string {
  ...
  if (cell?.type === 'entries' && cell.entries) {
    return cell.entries.map((entry: any) => {
      if (typeof entry === 'string') {
        return `<div>${processFormatting(entry)}</div>`
      }
      if (entry?.entries) {
        return entry.entries.map((e: any) => 
          typeof e === 'string' ? `<div>${processFormatting(e)}</div>` : ''
        ).join('')
      }
      return ''
    }).join('')
  ...
  return processFormatting(String(cell))
}