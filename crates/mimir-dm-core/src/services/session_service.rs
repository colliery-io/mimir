...
impl SessionService {
    ...
    pub fn create_session(
        ...
        let initial_status = stages.first().ok_or_else(|| {
            diesel::result::Error::QueryBuilderError("Session board has no stages defined".into())
        })?;
    ...
    fn render_template(
        ...
        tera.add_raw_template("template", &template.document_content)
            .map_err(|e| {
                diesel::result::Error::QueryBuilderError(format!("Template error: {}", e).into())
            })?;
        ...
        let rendered = tera.render("template", &context).map_err(|e| {
            diesel::result::Error::QueryBuilderError(format!("Render error: {}", e).into())
        })?;
        Ok(rendered)
    }
