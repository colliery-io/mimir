...
impl ActionService {
    ...
    pub fn search_actions(
        ...
        let actions: Vec<CatalogAction> = query.order(catalog_actions::name.asc()).load(conn)?;
    ...
    pub fn get_action_by_id(conn: &mut SqliteConnection, action_id: i32) -> Result<Option<Action>> {
    ...
    pub fn get_action_count(conn: &mut SqliteConnection) -> Result<i64> {
        ...
        let count: i64 = catalog_actions::table.select(count_star()).first(conn)?;
