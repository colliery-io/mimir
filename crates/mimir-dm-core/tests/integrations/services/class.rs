...
fn seed_test_class_data(conn: &mut SqliteConnection) {
    // Insert base classes
    let classes = vec![
        ("Wizard", "PHB", "d6", Some("int"), Some("Intelligence"), r#"{"name":"Wizard","source":"PHB","hd":{"number":1,"faces":6},"casterProgression":"full","spellcastingAbility":"int"}"#),
        ("Fighter", "PHB", "d10", None, Some("Strength or Dexterity"), r#"{"name":"Fighter","source":"PHB","hd":{"number":1,"faces":10}}"#),
        ("Cleric", "PHB", "d8", Some("wis"), Some("Wisdom"), r#"{"name":"Cleric","source":"PHB","hd":{"number":1,"faces":8},"casterProgression":"full","spellcastingAbility":"wis"}"#),
        ("Rogue", "PHB", "d8", None, Some("Dexterity"), r#"{"name":"Rogue","source":"PHB","hd":{"number":1,"faces":8}}"#),
        ("Paladin", "PHB", "d10", Some("cha"), Some("Strength and Charisma"), r#"{"name":"Paladin","source":"PHB","hd":{"number":1,"faces":10},"casterProgression":"half","spellcastingAbility":"cha"}"#),
        ("Barbarian", "PHB", "d12", None, Some("Strength"), r#"{"name":"Barbarian","source":"PHB","hd":{"number":1,"faces":12}}"#),
        ("Blood Hunter", "TDCSR", "d10", None, Some("Strength or Dexterity"), r#"{"name":"Blood Hunter","source":"TDCSR","hd":{"number":1,"faces":10}}"#),
    ];
    ...
    // Insert subclasses
    let subclasses = vec![
        ("School of Evocation", "Wizard", "PHB", "PHB", Some("int"), r#"{"name":"School of Evocation","shortName":"Evocation","source":"PHB","className":"Wizard","classSource":"PHB"}"#),
        ("School of Necromancy", "Wizard", "PHB", "PHB", Some("int"), r#"{"name":"School of Necromancy","shortName":"Necromancy","source":"PHB","className":"Wizard","classSource":"PHB"}"#),
        ("Champion", "Fighter", "PHB", "PHB", None, r#"{"name":"Champion","shortName":"Champion","source":"PHB","className":"Fighter","classSource":"PHB"}"#),
        ("Battle Master", "Fighter", "PHB", "PHB", None, r#"{"name":"Battle Master","shortName":"Battle Master","source":"PHB","className":"Fighter","classSource":"PHB"}"#),
        ("Life Domain", "Cleric", "PHB", "PHB", Some("wis"), r#"{"name":"Life Domain","shortName":"Life","source":"PHB","className":"Cleric","classSource":"PHB"}"#),
        ("Thief", "Rogue", "PHB", "PHB", None, r#"{"name":"Thief","shortName":"Thief","source":"PHB","className":"Rogue","classSource":"PHB"}"#),
        ("Echo Knight", "Fighter", "PHB", "EGW", None, r#"{"name":"Echo Knight","shortName":"Echo Knight","source":"EGW","className":"Fighter","classSource":"PHB"}"#),
    ];
...
fn test_search_classes_no_filters() {
    ...
    let results = service.search_classes(filters)
...
fn test_search_classes_by_name() {
    ...
    assert!(results.len() >= 1, "Should find Wizard");
...
fn test_search_classes_with_spellcasting() {
    ...
    assert!(results.len() >= 3, "Should return at least 3 spellcasting classes");
}
...
fn test_search_classes_without_spellcasting() {
    ...
    assert!(results.len() >= 4, "Should return at least 4 non-spellcasting classes");
}
...
fn test_get_class_by_name_and_source() {
    ...
    let class = service.get_class_by_name_and_source("Wizard", "PHB")
...
fn test_get_class_by_name_and_source_not_found() {
    ...
    let class = service.get_class_by_name_and_source("Nonexistent", "PHB")
...
fn test_get_subclasses_for_class() {
    ...
    let subclasses = service.get_subclasses_for_class("Fighter", "PHB")
        .expect("Should get subclasses");

    // We seeded 3 Fighter subclasses in test data
    assert!(subclasses.len() >= 3, "Fighter should have at least 3 subclasses");
}
...
fn test_get_class_sources() {
    ...
    let sources = service.get_class_sources()
...
fn test_get_primary_abilities() {
    ...
    let abilities = service.get_primary_abilities()
...
fn test_get_class_count_by_source() {
    ...
    let counts = service.get_class_count_by_source()
