...
fn seed_test_background_data(conn: &mut SqliteConnection) {
    ...
    let backgrounds = vec![
        ("Acolyte", "PHB", "Insight, Religion", "None", "Shelter of the Faithful", r#"{"name":"Acolyte","source":"PHB","skillProficiencies":[{"insight":true,"religion":true}],"feature":{"name":"Shelter of the Faithful"}}"#),
        ("Criminal", "PHB", "Deception, Stealth", "Thieves' tools", "Criminal Contact", r#"{"name":"Criminal","source":"PHB","skillProficiencies":[{"deception":true,"stealth":true}],"toolProficiencies":[{"thieves' tools":true}],"feature":{"name":"Criminal Contact"}}"#),
        ("Folk Hero", "PHB", "Animal Handling, Survival", "Vehicles (land)", "Rustic Hospitality", r#"{"name":"Folk Hero","source":"PHB","skillProficiencies":[{"animal handling":true,"survival":true}],"toolProficiencies":[{"vehicles (land)":true}],"feature":{"name":"Rustic Hospitality"}}"#),
        ("Noble", "PHB", "History, Persuasion", "Gaming set", "Position of Privilege", r#"{"name":"Noble","source":"PHB","skillProficiencies":[{"history":true,"persuasion":true}],"toolProficiencies":[{"gaming set":true}],"feature":{"name":"Position of Privilege"}}"#),
        ("Sage", "PHB", "Arcana, History", "None", "Researcher", r#"{"name":"Sage","source":"PHB","skillProficiencies":[{"arcana":true,"history":true}],"feature":{"name":"Researcher"}}"#),
        ("Soldier", "PHB", "Athletics, Intimidation", "Vehicles (land)", "Military Rank", r#"{"name":"Soldier","source":"PHB","skillProficiencies":[{"athletics":true,"intimidation":true}],"toolProficiencies":[{"vehicles (land)":true}],"feature":{"name":"Military Rank"}}"#),
        ("Urchin", "PHB", "Sleight of Hand, Stealth", "Thieves' tools, Disguise kit", "City Secrets", r#"{"name":"Urchin","source":"PHB","skillProficiencies":[{"sleight of hand":true,"stealth":true}],"toolProficiencies":[{"thieves' tools":true,"disguise kit":true}],"feature":{"name":"City Secrets"}}"#),
        ("Haunted One", "VRGR", "Arcana, Investigation", "None", "Heart of Darkness", r#"{"name":"Haunted One","source":"VRGR","skillProficiencies":[{"arcana":true,"investigation":true}],"feature":{"name":"Heart of Darkness"}}"#),
        ("Far Traveler", "SCAG", "Insight, Perception", "Musical instrument", "All Eyes on You", r#"{"name":"Far Traveler","source":"SCAG","skillProficiencies":[{"insight":true,"perception":true}],"toolProficiencies":[{"musical instrument":true}],"feature":{"name":"All Eyes on You"}}"#),
        ("Outlander", "PHB", "Athletics, Survival", "Musical instrument", "Wanderer", r#"{"name":"Outlander","source":"PHB","skillProficiencies":[{"athletics":true,"survival":true}],"toolProficiencies":[{"musical instrument":true}],"feature":{"name":"Wanderer"}}"#),
    ];
...
fn test_search_backgrounds_without_tools() {
    ...
    assert!(results.len() >= 3, "Should return backgrounds without tools");
}
...
fn test_search_backgrounds_combined_filters() {
    ...
    assert!(results.len() >= 5, "Should return PHB backgrounds with tools");
    assert!(results.iter().all(|b| b.source == "PHB"));
}
