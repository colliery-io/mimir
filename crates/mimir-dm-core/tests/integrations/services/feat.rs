...
fn test_get_feat_by_name_and_source() {
    ...
    let feat = FeatService::get_feat_by_name_and_source(&mut conn, "Lucky", "PHB")
...
fn test_get_feat_by_name_and_source_not_found() {
    ...
    let result = FeatService::get_feat_by_name_and_source(&mut conn, "Nonexistent", "PHB");

    assert!(result.is_err(), "Should error for nonexistent feat");
}
