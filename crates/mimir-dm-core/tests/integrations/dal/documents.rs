...
fn test_create_document() {
    ...
    // Create a template
    let template = TemplateRepository::create(&mut conn, NewTemplateDocument {
    ...
    let doc = DocumentRepository::create(&mut conn, new_doc)
...
fn test_find_document_by_id() {
    ...
    let doc = DocumentRepository::create(&mut conn, NewDocument {
        campaign_id: campaign.id,
        module_id: None,
        session_id: None,
    ...
    let found = DocumentRepository::find_by_id(&mut conn, doc.id)
...
fn test_find_documents_by_campaign() {
    ...
    for i in 1..=3 {
        DocumentRepository::create(&mut conn, NewDocument {
...
fn test_update_document() {
    ...
    let updated = DocumentRepository::update(&mut conn, doc.id, update)
...
fn test_find_incomplete_and_completed() {
    ...
    for i in 1..=2 {
        ...
        DocumentRepository::mark_completed(&mut conn, doc.id)
...
fn test_delete_document() {
    ...
    let deleted = DocumentRepository::delete(&mut conn, doc.id)
...
fn test_document_level() {
    ...
    // Test campaign level
    let camp_doc = DocumentRepository::create(&mut conn, NewDocument {
        campaign_id: campaign.id,
        module_id: None,
        session_id: None,
        template_id: template.document_id.clone(),
    ...
    // Test module level
    let mod_doc = DocumentRepository::create(&mut conn, NewDocument {
        campaign_id: campaign.id,
        module_id: Some(module.id),
        session_id: None,
        template_id: template.document_id.clone(),
    ...
    // Test session level
    let sess_doc = DocumentRepository::create(&mut conn, NewDocument {
        campaign_id: campaign.id,
        module_id: Some(module.id),
        session_id: Some(session.id),
        template_id: template.document_id.clone(),
    ...
    // Test handout level
    let handout_doc = DocumentRepository::create(&mut conn, NewDocument {
        campaign_id: campaign.id,
        module_id: None,
        session_id: None,
        template_id: template.document_id.clone(),
...
fn test_exists_by_path() {
    ...
    let _doc = DocumentRepository::create(&mut conn, NewDocument {
        campaign_id: campaign.id,
        module_id: None,
        session_id: None,
        template_id: template.document_id,
    ...
    assert!(DocumentRepository::exists_by_path(&mut conn, "/test/check_path.md")
        .expect("Failed to check existence"));
    assert!(!DocumentRepository::exists_by_path(&mut conn, "/test/nonexistent.md")
        .expect("Failed to check existence"));
}