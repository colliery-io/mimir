...
fn test_session_lifecycle() {
    ...
    let updated = session_repo
        .transition_status(session.id, "prep_needed")
        .unwrap();
    assert_eq!(updated.status, "prep_needed");

    let updated = session_repo
        .transition_status(session.id, "in_prep")
        .unwrap();
    ...
    let updated = session_repo
        .transition_status(session.id, "complete")
        .unwrap();
...
fn test_find_sessions_needing_prep() {
    ...
    let sessions = session_repo
        .find_sessions_needing_prep(campaign.id, 3)
        .unwrap();
    ...
    let sessions = session_repo
        .find_sessions_needing_prep(campaign.id, 5)
        .unwrap();
